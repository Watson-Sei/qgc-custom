...
**初回起動時にカメラ権限のダイアログが出ます。** Qt の macOS カメラ実装
(`QAVFCameraBase::checkCameraPermission`) は権限を*確認するだけで要求しない*ため、
未許可のままだとデバイス一覧には出るのに映像だけが出ない（黙って黒画面になる）という
状態になります。そのためパネル側で `CameraPermission.request()` を呼んでいます。
拒否してしまった場合はシステム設定 > プライバシーとセキュリティ > カメラ から許可し直してください
（パネルにもその旨を表示します）。なお ad-hoc 署名のビルドは再ビルドのたびに署名が変わるため、
許可を訊かれ直すことがあります。
...
- 待機中は丸、録画中は四角＋点滅。録画中は経過時間 `MM:SS` を表示
- 保存先は `~/Movies/QGC-HDMI-YYYYMMDD-HHMMSS.mp4`（MPEG-4 / H.264、音声なし）
- 停止すると `Saved: ファイル名` を数秒表示。失敗時は同じ場所にエラー内容を表示
- 録画中はデバイス切替を無効化し、**折りたたんでもカメラを解放しません**
  （解放すると録画が切れるため）。経過時間は折りたたみ中も見えます
- 録画される解像度は**キャプチャデバイスのソース解像度そのまま**です。
  プレビュー枠の 16:9 にクロップされるわけではありません
- `AudioInput` は意図的に接続していません。映像のみで、マイク権限も要求しません

`QGroundControl.videoManager` とは**独立**しています。ここでデバイスを選んでも
アプリケーション設定のビデオ設定は変わらず、機体側の映像と同時に表示できます。

**位置**: 右端ぴったり、純正インストゥルメントパネルの真下に置いています
（`src/qml/FlyViewCustomLayer.qml` の `rightEdgeColumn`）。

なお upstream の `FlyViewTopRightPanel` は**非表示でも領域を予約する**ため、
機体が繋がっていない・テレメトリ値が出ない状態だとパネルの上に空白ができます。
常に画面上端に寄せたい場合は `anchors.topMargin` から
`parentToolInsets.topEdgeRightInset` を外してください（純正パネルと重なります）。
